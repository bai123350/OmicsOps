...
| Skills | `SettingsPanel.tsx:442`的SkillCard呈现名称/版本/摘要片段/能力并调用启停；`src/tauri-api.ts:288`一组API连接list/import/enable | S1来源receipt `d7ba5e9`已提交；父junction修复及S2/S3详情/文件/安全删除/启停反馈仍在进行，整页未完成。 |
...
Connections、Usage、Environments基础页已提交；General、Skills、Plugins仍未完成。`13d9c57`时Rust workspace全套通过，后续新增之后仍需重跑。新增提交后的最终全套确定性检查仍待统一重跑；真实SSH/model/PBMC验收没有由这些测试代替。某次Vitest阶段711项通过，但npm参数被末尾node --test解析导致整体命令失败，不能称完整npm test成功。
...
| General G1 `b67f2c2` / `75bdf7e` | native 7/7及desktop build、UI 72/72及Web build通过；目录失效宿主预检/ProjectLibrary notice仍在补，G2/G3未完成。 |
| Environments `c306e0a` | system probe、新SSH入口、字段busy、绑定反馈及防旧绑定拉回项目已接入；Settings 64、Settings + Desktop 142/142及Web build通过。 |
| Skills S1 `d7ba5e9` | 来源receipt adapter 2/2、native 16/16、fmt通过；父junction修复和S2/S3仍进行，不能标整页完成。 |
