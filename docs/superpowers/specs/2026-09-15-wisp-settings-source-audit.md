...
实施跟踪补充：下表“OmicsOps当前实现”保留审计基线语义；其后的实现已在 `docs/superpowers/plans/2026-09-15-settings-center.md` 的master和提交记录更新，不能用此基线表推断当前仍未实现。剩余Credentials、Usage及Plugins/General已各有同目录独立执行计划。

基础适配边界已锁定：Specialists交付用户可见的项目角色草稿模板CRUD，独立专家执行与模型/工具约束不声称已完成；Memory交付项目记忆文件管理，全局习惯独立可选；Plugins交付真实本地声明包生命周期而非市场/脚本运行器；General目录为“新项目目录选择的起始位置”，缺更新源准确显示未配置，环境/网络入口连接现有真实管理与探测。以上不是Wisp全功能等价声明，也不能把仅说明或未接线按钮作为完成。
