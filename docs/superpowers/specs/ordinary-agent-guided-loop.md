...
对话展示采用连续正文与紧凑工具行，参考
[wisp-science 的聊天呈现](https://github.com/xuzhougeng/wisp-science/blob/main/ui/src/chat_render.rs)。
公开 `model_text` 直接呈现为 Markdown 段落，不重复机器人头像或“进度”标签；
`cycle_started` 不再生成历史“思考中”行。工具调用按首次事件 sequence 插入，
后续 dispatch/outcome 合并到同一行。
读、写、编辑保留独立操作名，批次不能隐藏其中的单项调用。工具行提供状态、路径/命令、
可用的事件耗时与输出行数，输入输出独立展开。任务、阶段及批次汇总收进附加详情。
正文、工具行、问题、审批、异常原因和恢复按钮始终位于执行详情之外，历史会话也不折叠正文。
执行详情保留状态、耗时与实际工具数量；内部归档、checkpoint 等事件仅进入默认折叠的
诊断记录（序号、事件类型、时间，不输出 payload）。主对话采用明确的用户事件白名单，
新增基础设施事件不会自动成为助手消息。`run_needs_attention` 显示本地化标题和原始公开
message，不补造答复或科研结论。最终答复沿用既有持久化消息，不将完成提案直接当作结果。
活动执行详情默认展开，历史详情默认折叠；向上阅读时暂停自动滚动，用户可返回最新。
该展示在 Windows/macOS 使用同一事件协议，不改变审批、恢复、凭据或数据库边界。

2026-09-10 验证：WorkspaceShell 覆盖中英文 attention、归档诊断折叠、进度/工具顺序、
历史正文与最终答复可见性，既有审批、恢复和流式合并测试继续适用。手工 smoke：在 Windows
桌面执行一次检索，检查进度与工具明细，折叠执行过程后确认问题/审批仍可操作；切换会话后
确认历史正文和最终答复可读。此次仅调整前端呈现，未执行真实模型或 SSH 验收，macOS 未实测。
实际检查通过：`cargo test --workspace`（真实环境用例仍 ignored）、`npm test`
（126 项前端、22 项扩展测试）、`npm run build`（仍有大于 500 kB 的 chunk 提示）。
另用本地浏览器和模拟事件检查正文、失败原因、默认折叠诊断及工具输入输出展开；
临时预览文件已移除，该检查不等同于原生桌面或真实科研流程验收。
