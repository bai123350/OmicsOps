use super::{
    ...
    DEFAULT_PAGE_SIZE, MAX_ANTIBODY_RECORDS, MAX_PAGE_SIZE, MAX_QUERY, antibody_get,
    antibody_origin, bound_u32, get_json, json_text, json_u64, require_text,
};
use crate::NativeBio;
...
pub(super) fn parse_ab_id(value: &str) -> Result<u64> {
    ...
    if trimmed.is_empty() || trimmed.len() > 32 {
        bail!(
            "antibody_id must be an Antibody Registry accession such as 3643095, AB_3643095, or RRID:AB_3643095"
        );
    }
    let rest = trimmed
        .strip_prefix("RRID:")
