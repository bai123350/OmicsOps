use super::{
    DEFAULT_GRANT_RECORDS, GRANTS, GRANTS_SITE, MAX_GRANT_RECORDS, MAX_QUERY, bound_u32,
    grants_origin, json_text, json_u64, pipe_join, require_text,
};
use crate::NativeBio;
...
impl Spec {
    fn from_args(args: &Search) -> Result<Self> {
        ...
        {
            bail!(
                "search_grants requires at least one of keyword, opportunity_number, aln, agencies, eligibilities, funding_categories or funding_instruments"
            );
        }
        Ok(Self {
            keyword,
