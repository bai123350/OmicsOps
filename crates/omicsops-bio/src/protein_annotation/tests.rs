...
fn rejects_unbounded_or_malformed_arguments() {
    ...
    assert!(
        serde_json::from_value::<interpro::DomainArgs>(
            json!({"accessions": ["P04637"], "api_key": "secret"})
        )
        .is_err()
    );
    assert!(
        serde_json::from_value::<interpro::SearchEntries>(json!({"query": "kinase", "token": "x"}))
            .is_err()
    );
    assert!(
        serde_json::from_value::<protein_atlas::GeneArgs>(json!({"gene": "TP53", "api_key": "x"}))
            .is_err()
    );
    assert!(
        serde_json::from_value::<string::MapArgs>(json!({"symbols": ["TP53"], "api_key": "x"}))
            .is_err()
    );
