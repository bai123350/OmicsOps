...
fn catalog_registers_twelve_chemistry_tools() {
    ...
    let tools = crate::catalog()
        .into_iter()
        .filter(|(domain, _)| crate::package_selects("mcp_chemistry", domain))
        .collect::<Vec<_>>();
...
async fn argument_bounds_and_unknown_fields() {
    ...
    assert!(
        super::pubchem::get_compounds(&bio, &json!({"cids": []}))
            .await
            .is_err()
    );
    assert!(
        super::pubchem::get_compounds(&bio, &json!({"cids": [0]}))
            .await
            .is_err()
    );
    assert_eq!(
    ...
    assert!(
        super::bindingdb::targets_by_compound(&bio, &json!({"smiles": "CCO", "similarity": 0.1}))
            .await
            .is_err()
    );
}
