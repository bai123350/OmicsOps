...
const INVESTIGATOR_FIELDS: &str = "NCTId|BriefTitle|Condition|ContactsLocationsModule|ResponsiblePartyInvestigatorFullName|ResponsiblePartyInvestigatorAffiliation|ResponsiblePartyType";
...
fn eligibility_params(args: &SearchEligibility) -> Result<Vec<(String, String)>> {
    ...
    {
        bail!(
            "search_by_eligibility requires condition, eligibility_keywords, min_age, max_age or sex"
        );
    }
    let mut params = Vec::new();
...
fn investigator_params(args: &SearchInvestigators) -> Result<Vec<(String, String)>> {
    ...
    {
        bail!(
            "search_investigators requires investigator_name, institution, location, condition or status"
        );
    }
    let mut params = Vec::new();
    push_query(&mut params, "query.cond", condition);
