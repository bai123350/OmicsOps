use super::{
    EQTL_SITE, MAX_EQTL, bound_records, default_page, eqtl_files, optional_text, require_ensg,
...
pub(super) async fn associations(bio: &NativeBio, args: &Value) -> Result<Value> {
    ...
    if filters.is_empty() {
        bail!(
            "pass at least one of gene_id / rsid / variant / pos — the API rejects unfiltered association scans"
        );
    }
