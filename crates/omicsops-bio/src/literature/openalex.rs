...
use super::{NativeBio, OPENALEX, bound, encode_segment, short_id, trimmed};
...
pub(super) fn normalize_author_id(author_id: &str) -> Result<String> {
    ...
    bail!(
        "unrecognized author id {author_id:?} — pass an OpenAlex A-id, an openalex.org URL, or an ORCID"
    )
}
...
pub(super) fn normalize_source_id(source_id: &str) -> Result<String> {
    ...
    bail!(
        "unrecognized source id {source_id:?} — pass an OpenAlex S-id, an openalex.org URL, or an ISSN"
    )
}
