...
impl Http {
    ...
    pub fn with_proxy(proxy: &str) -> Result<Self> {
        let builder = reqwest::Client::builder()
            .user_agent(concat!("OmicsOps-science/", env!("CARGO_PKG_VERSION")))
...
mod tests {
    ...
    #[tokio::test]
    async fn failed_submission_is_not_retried_or_exposed_in_errors() {
        ...
        let app = Router::new().route(
            ...
            axum::routing::post(move || {
                ...
                async move {
                    ...
                    (StatusCode::SERVICE_UNAVAILABLE, "secret echoed by upstream")
                }
            }),
        );
        ...
        let http = Http::with_proxy("none").unwrap();
        let response = http
            .send_json(
                Source("submission fixture", Duration::ZERO),
                &format!("http://{address}/submit?api_key=secret"),
                &serde_json::json!({}),
            )
            .await
            .unwrap();
        let error = response.check().unwrap_err().to_string();
    ...
    async fn downloads_follow_local_hops_but_reject_foreign_redirects_and_bad_ranges() {
        ...
        assert!(
            http.ebi_download(source, &format!("{base}/foreign"))
                .await
                .err()
                .unwrap()
                .to_string()
        ...
        assert!(
            http.range(source, &format!("{base}/bad-range"), 5, 7)
                .await
                .is_err()
        );
        assert!(
            http.range(source, &format!("{base}/data"), 5, 7)
                .await
                .is_err()
        );
        server.abort();
