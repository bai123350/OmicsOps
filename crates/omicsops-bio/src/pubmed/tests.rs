...
fn catalog_uses_deferred_read_only_tools() {
    ...
    for (_, tool) in &tools {
        assert!(names.insert(tool.function.name.clone()));
    }
    assert_eq!(names.len(), crate::catalog().len());
    let pubmed_only = crate::catalog()
        .into_iter()
        .filter(|(domain, _)| crate::package_selects("mcp_pubmed", domain))
        .collect::<Vec<_>>();
...
fn convert_encodes_same_type_batches_and_reports_missing_embargoed_and_unconverted() {
    ...
    assert!(
        result["unconverted_ids"][1]["reason"]
            .as_str()
            .unwrap()
