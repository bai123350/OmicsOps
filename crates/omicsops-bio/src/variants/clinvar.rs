use super::{
    BATCH_DEADLINE, CLINVAR_BROWSER, NativeBio, clinvar_url, json_i64, json_string, ncbi_json,
    page_bound, require_rsid, require_text,
};
...
pub(super) async fn get_records(bio: &NativeBio, args: &Value) -> Result<Value> {
    ...
    for acc in &pending {
        if let Some(uid) = vcv_uid(acc) {
            ...
            bail!(
                "unrecognized ClinVar accession {acc:?} (expected VCVnnn, RCVnnn, or a variation ID)"
            );
        }
    }
