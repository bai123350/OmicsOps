...
fn clinvar_gold_stars_follow_official_review_status() {
    ...
    assert!(
        serde_json::from_value::<gnomad::GetVariant>(json!({
...
fn cadd_rejects_bare_version_mito_ref_eq_alt_span_and_unknown_fields() {
    ...
    assert!(
        serde_json::from_value::<cadd::PositionScores>(json!({
            "chrom": "4", "pos": 9001, "api_key": "secret"
        }))
        .is_err()
    );
    assert!(
        serde_json::from_value::<cadd::VariantScore>(json!({
            "chrom": "4", "pos": 9001, "ref": "A", "alt": "T", "api_key": "secret"
        }))
        .is_err()
    );
    assert!(
        serde_json::from_value::<cadd::RangeScores>(json!({
