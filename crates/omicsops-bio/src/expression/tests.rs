...
fn rejects_unbounded_or_malformed_arguments() {
    assert!(
        serde_json::from_value::<ResolveGenes>(json!({"gene_ids": ["GAPDH"], "api_key": "secret"}))
            .is_err()
    );
...
fn paging_and_unversioned_helpers_distinguish_empty_from_malformed() {
    ...
    assert!(
        parse_page(&json!({"data": [1], "paging_info": {
...
fn panglaodb_rejects_invalid_arguments() {
    assert!(
        serde_json::from_value::<panglaodb::MarkerGenes>(
    ...
    assert!(
        serde_json::from_value::<panglaodb::CellTypesForGene>(
