...
impl Store {
    ...
    pub async fn reserve_runtime_job_v4(
        ...
        self.reserve_runtime_job_with_remote_root_v4(context, call_id, request_sha256, None, None)
            .await
    }
    ...
    pub async fn reserve_runtime_job_with_remote_root_v4(
        ...
        remote_host_key: Option<&str>,
    ) -> Result<(RuntimeJobV4, bool), StoreError> {
        if remote_root.is_some() != remote_host_key.is_some()
            || remote_host_key.is_some_and(|key| key.trim().is_empty())
            || remote_root.is_some_and(|root| {
                !root.starts_with('/') || root == "/" || !context.backend_id.starts_with("ssh:")
            })
        {
            return Err(StoreError::InvalidInput(
                "detached jobs require an absolute SSH root".into(),
            ));
        }
        ...
        if let Some(previous) = previous {
            ...
            if job.context != *context
                ...
                || job.remote_root.as_deref() != remote_root
                || job.remote_host_key.as_deref() != remote_host_key
            {
                return Err(StoreError::InvalidInput(
