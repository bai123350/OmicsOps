...
impl Store {
    pub async fn accept_guidance_v4(
        &self,
    ...
    pub async fn list_guidance_v4(
        ...
        run_id: Uuid,
    ) -> Result<Vec<GuidanceRecordV4>, StoreError> {
    ...
    pub async fn consume_guidance_v4(
        &self,
        expected: &RunSpecV4,
    ) -> Result<Vec<AgentEventV4>, StoreError> {
        let mut tx = self.pool.begin_with("BEGIN IMMEDIATE").await?;
        let (_, spec) = guidance_spec(
            &mut tx,
            expected.project_id,
        ...
        if spec != *expected {
            return Err(StoreError::InvalidInput(
                "guidance consumer has a different frozen specification".into(),
            ));
        }
        ...
        if history.iter().any(is_terminal_event) {
            return Err(StoreError::InvalidInput(
                "cannot consume guidance after the run stopped".into(),
            ));
        }
        let mut previous = history
            .last()
            .cloned()
            .ok_or_else(|| StoreError::InvalidInput("run has not started".into()))?;
...
async fn guidance_spec(
    ...
    conversation_id: Uuid,
    run_id: Uuid,
) -> Result<(String, RunSpecV4), StoreError> {
    ...
    let spec: RunSpecV4 = serde_json::from_value(
        value
            .get("spec")
            .cloned()
            .ok_or_else(|| StoreError::InvalidInput("run has no frozen specification".into()))?,
    )?;
    ...
    {
        return Err(StoreError::InvalidInput(
            "guidance cannot modify an approved plan or another run".into(),
        ));
    }
...
fn guidance_record(
    ...
    Ok(GuidanceRecordV4 {
        ...
        run_id: spec.run_id,
        ordinal: row.try_get::<i64, _>(1)? as u64,
        markdown: row.try_get(2)?,
        accepted_at: from_timestamp(row.try_get(3)?, "guidance accepted timestamp")?,
        consumed_at: row
            .try_get::<Option<i64>, _>(4)?
            .map(|value| from_timestamp(value, "guidance consumed timestamp"))
            .transpose()?,
    })
}
