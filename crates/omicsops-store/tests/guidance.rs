...
async fn guidance_acceptance_is_idempotent_ordered_and_scope_checked() {
    ...
    let (a, b) = tokio::join!(
        store.accept_guidance_v4(&first),
        store.accept_guidance_v4(&first)
    );
    let a = a.unwrap();
    ...
    conflicting.markdown = "different".into();
    assert!(store.accept_guidance_v4(&conflicting).await.is_err());
    conflicting = first.clone();
    conflicting.conversation_id = Uuid::new_v4();
    assert!(store.accept_guidance_v4(&conflicting).await.is_err());
    assert!(
        store
            .list_guidance_v4(Uuid::new_v4(), spec.conversation_id, spec.run_id)
            .await
            .is_err()
    );
    let second = store
        .accept_guidance_v4(&request(&spec, "state uncertainty"))
        .await
        .unwrap();
    ...
    assert!(
        matches!(&consumed[0].event, AgentEventKindV4::GuidanceConsumed {message_id, ..} if *message_id == first.message_id)
    );
    assert!(store.consume_guidance_v4(&spec).await.unwrap().is_empty());
    assert!(!store.has_pending_guidance_v4(spec.run_id).await.unwrap());
    assert!(
        store
            .list_guidance_v4(spec.project_id, spec.conversation_id, spec.run_id)
            .await
            .unwrap()
            .iter()
            .all(|row| row.consumed_at.is_some())
    );
}
...
async fn guidance_survives_reopen_and_consumption_rolls_back_with_event_failure() {
    ...
    let path = dir.path().join("guidance.sqlite");
    let store = Store::open(&path).await.unwrap();
    let spec = fixture(&store, true).await;
    store
        .accept_guidance_v4(&request(&spec, "durable guidance"))
        .await
        .unwrap();
    ...
    assert!(store.has_pending_guidance_v4(spec.run_id).await.unwrap());
    assert_eq!(store.agent_events_v4(spec.run_id).await.unwrap().len(), 1);
    sqlx::query("DROP TRIGGER reject_guidance")
        .execute(store.pool())
        .await
        .unwrap();
    let (a, b) = tokio::join!(
        store.consume_guidance_v4(&spec),
        store.consume_guidance_v4(&spec)
    );
    ...
    assert_eq!(
        reopened.agent_events_v4(spec.run_id).await.unwrap().len(),
        2
    );
}
...
async fn pending_guidance_prevents_completion_and_terminal_runs_reject_new_input() {
    ...
    assert!(matches!(
        store.append_agent_event_v4(&done).await,
        Err(StoreError::GuidancePending)
    ));
    assert!(
        store
            .messages_for_conversation(spec.conversation_id)
            .await
            .unwrap()
            .is_empty()
    );
    let consumed = store.consume_guidance_v4(&spec).await.unwrap();
    let done = AgentEventV4::next(
    ...
    assert!(
        store
            .accept_guidance_v4(&request(&spec, "too late"))
            .await
            .is_err()
    );
    assert!(
        store
            .accept_guidance_v4(&req)
            .await
            .unwrap()
            .consumed_at
            .is_some()
...
async fn guidance_and_completion_race_has_only_one_winner() {
    for _ in 0..4 {
        ...
        let (accepted, completed) = tokio::join!(
            store.accept_guidance_v4(&req),
            store.append_agent_event_v4(&done)
        );
        assert_ne!(accepted.is_ok(), completed.is_ok());
        if accepted.is_ok() {
            assert!(matches!(completed, Err(StoreError::GuidancePending)));
        }
    }
}
...
async fn guidance_rejects_plans_and_enforces_byte_and_lifetime_limits() {
    let store = Store::open_in_memory().await.unwrap();
    let plan = fixture(&store, false).await;
    assert!(
        store
            .accept_guidance_v4(&request(&plan, "cannot change a plan"))
            .await
            .is_err()
    );
    let spec = fixture(&store, true).await;
    assert!(
        store
            .accept_guidance_v4(&request(&spec, &"测".repeat(683)))
            .await
            .is_err()
    );
    assert!(
        store
            .accept_guidance_v4(&request(&spec, "  "))
            .await
            .is_err()
    );
    for index in 0..16 {
        store
            .accept_guidance_v4(&request(&spec, &format!("guidance {index}")))
            .await
            .unwrap();
    }
    ...
    assert!(
        store
            .accept_guidance_v4(&request(&spec, "seventeenth"))
            .await
            .is_err()
    );
}
...
async fn simultaneous_new_runs_cannot_acquire_the_same_conversation() {
    ...
    let value = json!({});
    let (a, b) = tokio::join!(
        store.save_agent_run_v4_if_unlocked(
            Uuid::new_v4(),
            spec.project_id,
            spec.conversation_id,
            "running",
