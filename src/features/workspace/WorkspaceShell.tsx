...
import type { AgentRunEventV3, AgentRunEventV4, AgentRunStreamEvent, ApprovalPolicyV4, AutonomyModeV4, ComputeBackendAvailabilityV4, FormalStepProposal, KernelEvent, KernelLanguage, KernelSession, MemoryFact, NotebookEntry, PlanProposal, ProjectArtifact, ProjectImagePreview, RunSummaryV4, SyncEntry, WorkspaceConversation } from "../../types";
...
export function WorkspaceShell({ project, locale, onLocaleChange, onOpenSettings, onBackToProjects, conversations = [], activeConversationId, onSelectConversation, onNewConversation, onDeleteConversation, onSend, messages = [], streamingAssistant = "", agentBusy = false, agentNotice = "", agentRetryNotice = "", modelLabel, planProposal, v4Plan, computeBackends = [], computeBackendId = "", containerImage = "", autonomyMode = "supervised", approvalPolicy = "risk_based", computeEnvironment = "system", computeBusy = false, onComputeBackendChange, onContainerImageChange, onAutonomyModeChange, onApprovalPolicyChange, onComputeEnvironmentChange, planLoading = false, planApproved = false, onRequestPlan, onApprovePlan, onStartRun, onCancelRun, runStopping = false, canStartRun = false, runStarted = false, activeRunId, agentRunEvents = [], agentRunEventsV3 = [], agentRunEventsV4 = [], onAnswerAgentQuestionV3, onAnswerAgentQuestionV4, onResumeAgentRunV4, remoteFiles, filesBusy = false, onUploadFiles, onRefreshFiles, onDownloadFile, onPreviewImage, fileNotice, kernelSessions = [], kernelEvents = [], kernelBusy = false, kernelNotice, onStartKernel, onExecuteKernel, onInterruptKernel, onStopKernel, onPromoteKernelCell, memoryFacts = [], notebookEntries = [], projectArtifacts = [], onSearchMemory, onExportNotebook, syncEntries = [], onPauseSync, onCancelSync, onRetrySync }: Props) {
  ...
  const runFinished = activeRunEvents.some((event) => event.kind === "agent_completed" || event.kind === "agent_failed" || event.kind === "agent_canceled") || activeRunEventsV3.some(isTerminalAgentEventV3) || Boolean(activeRunEventsV4.at(-1) && isTerminalAgentEventV4(activeRunEventsV4.at(-1)!));
    ...
    <main className="conversation-pane" aria-label={t.research}>
      ...
      <section className="message-stream" aria-live="polite" ref={messageStreamRef}>
        ...
        {messages.map((message) => <Fragment key={message.id}>
          ...
          {runTimelineV4.afterMessage.get(message.id)?.map((run) => <V4RunTrace locale={locale} events={run.events} onAnswer={onAnswerAgentQuestionV4} onResume={onResumeAgentRunV4} historical key={run.runId} />)}
        </Fragment>)}
        ...
        {runTimelineV4.unanchored.map((run) => <V4RunTrace locale={locale} events={run.events} onAnswer={onAnswerAgentQuestionV4} onResume={onResumeAgentRunV4} historical key={run.runId} />)}
        ...
        {visibleActiveRunEventsV4.length > 0 && <V4RunTrace locale={locale} events={visibleActiveRunEventsV4} onAnswer={onAnswerAgentQuestionV4} onResume={onResumeAgentRunV4} />}
      ...
      <footer className="composer">
        <div className={`composer-input ${planModeEnabled ? "is-plan-mode" : ""}`}>
          <textarea aria-label={t.composer} placeholder={planModeEnabled ? (zh ? "描述需要规划和执行的任务" : "Describe the task to plan and execute") : t.composer} value={draft} disabled={agentBusy} onChange={(event) => setDraft(event.target.value)} />
          <div>
            ...
            <button className="send-button" disabled={agentBusy || planLoading || !draft.trim() || Boolean(onSend && !computeReady)} onClick={send}><Send size={16} />{agentBusy || planLoading ? (planModeEnabled ? (zh ? "规划中…" : "Planning…") : (zh ? "执行中…" : "Running…")) : t.send}</button>
          </div>
        </div>
...
function V4RunTrace({ locale, events, onAnswer, onResume, historical = false }: { locale: Locale; events: AgentRunEventV4[]; onAnswer?: (runId: string, questionId: string, answer: string) => Promise<void> | void; onResume?: (runId: string) => Promise<void> | void; historical?: boolean }) {
  const zh = locale === "zh-CN";
