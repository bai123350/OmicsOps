...
export function SettingsPanel({ locale = "zh-CN", onLocaleChange, initialSection = "models", onClose, modelProfiles = [], onSaveModel, onDeleteModel, onProbeModel, onListModels, skillPackages = [], onImportSkill, onSetSkillEnabled, onSkillsChanged, mcpServers = [], onSaveMcpServer, onInspectMcpServer, onSetMcpServerEnabled, onSetMcpLaunchApproval, onSetMcpToolApproval, onListBundledMcpPresets, onConfigurePubMedMcp, onAddBundledMcp, connections = [], projects = [], selectedProject, onOpenUsageConversation, onSaveConnection, onTestConnection, onConfirmHostKey, onBindProjectRemote, onWorkflowsChanged, onMemoryChanged, onPluginsChanged }: Props) {
  ...
  async function deleteModel() {
    if (!deleteTarget || !onDeleteModel || deleting) return;
    const deletedProfileId = deleteTarget.id;
    setDeleting(true);
    setDeleteError("");
    try {
      await onDeleteModel(deletedProfileId);
      setDeleteTarget(null);
      setForm((current) => {
        if (!current || current.id === deletedProfileId) return null;
        return current.delegated_model_profile_id === deletedProfileId
          ? { ...current, delegated_model_profile_id: null }
          : current;
      });
    } catch (error) {
  ...
  return <div className="settings-backdrop"><section className="settings-panel" role="dialog" aria-modal="true" aria-label={zh ? "工作台设置" : "Workspace settings"}>
    ...
    {deleteTarget && <div className="model-delete-overlay"><section className="model-delete-dialog" role="alertdialog" aria-modal="true" aria-labelledby="model-delete-title"><h3 id="model-delete-title">{zh ? `删除“${deleteTarget.label}”？` : `Delete “${deleteTarget.label}”?`}</h3><p>{zh ? "这会删除保存的模型配置及其存储的凭据，并清除可变设置中的引用。已冻结的历史记录会保留；活动任务正在使用时无法删除。" : "This removes the saved profile and its stored credential, and clears references in mutable settings. Frozen history stays intact; deletion is blocked while active work uses this profile."}</p>{deleteError && <p className="model-delete-error" role="alert">{deleteError}</p>}<div><button disabled={deleting} onClick={() => { setDeleteTarget(null); setDeleteError(""); }}>{zh ? "取消" : "Cancel"}</button><button className="danger" disabled={deleting} onClick={() => void deleteModel()}>{deleting ? (zh ? "删除中…" : "Deleting…") : (zh ? "删除" : "Delete")}</button></div></section></div>}
  </section></div>;
}
