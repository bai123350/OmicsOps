...
新建模型配置或改变供应商、API 地址、模型 ID 时，会采用编译内置的 models.dev 能力快照，包含已知的上下文、输入和输出上限、工具/视觉能力与已公布的推理档位。目录覆盖 OpenAI、Anthropic、DeepSeek、Alibaba（国际/中国）、MiniMax（国际/中国）、OpenCode Go 及 OpenRouter 的文本输出模型。

匹配要求精确协议、HTTPS API 主机/端口和完整模型 ID；OpenCode Go 还要求精确的 `/zen/go/v1` 基础路径，OpenRouter 保留供应商前缀。不用型号家族、前缀或未知网关推断能力。未知网关沿用 32,768 上下文、4,096 输出回退，不推断视觉支持；既有 Ollama 视觉条目仍受支持。
